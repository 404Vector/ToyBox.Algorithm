...
for file in sources:
   ...
   testResultFilePath = RESULT_PATH + SwapExtension(file, "."+KEYWORD_TEST_FILE)
   ...
   if(not os.path.exists(testSourceFilePath)): 
      continue
   command = COMPILE_TEST_COMMAND.format(testResultFilePath, testSourceFilePath, resultFilePath)
   print(" - comfile >>" + command)
   os.system(command)
